...
mongoose.connect(MONGO_URI).then(() => {
  console.log('Connected to MongoDB (legacy server)');
}).catch((err) => {
  console.error('Failed to connect to MongoDB (legacy server):', err);
});
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    console.error('Register error (legacy):', err);
...
app.get('/api/admin/users', requireAdmin, async (_req, res) => {
  try {
    ...
    console.error('Fetch users error (legacy):', err);
    return res.status(500).json({ success: false, message: 'Internal server error' });
  }
});
...
app.delete('/api/admin/users/:id', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('Delete user error (legacy):', err);
    return res.status(500).json({ success: false, message: 'Internal server error' });
  }
});
...
app.listen(port, () => {
  console.log(`Legacy API server listening on http://localhost:${port}`);
});
